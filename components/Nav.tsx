  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200">
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-14">
        <Link href="/" className="font-display text-base font-semibold tracking-tight text-slate-900 hover:text-blue-900 transition">
          rickbarretto.com
        </Link>
              ...
              <li key={item.href}>
                <Link
                  ...
                  className={`px-2.5 py-1.5 rounded-md transition font-medium ${
                    isActive ? "bg-blue-900 text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"
                  }`}
                >
                  {item.label}
                </Link>
